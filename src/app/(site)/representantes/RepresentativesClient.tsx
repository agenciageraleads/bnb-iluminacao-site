...
function buildTerritoryGroups(representatives: Representative[], selectedState: string) {
    const groups = new Map<string, { label: string; priority: number; rows: TerritoryRepresentative[] }>();
    ...
    return [...groups.entries()]
        ...
        .sort((a, b) => a.priority - b.priority || a.label.localeCompare(b.label, 'pt-BR'));
}
                                        ...
                                        <div key={group.key} className="space-y-3">
                                            ...
                                            <div className="space-y-4">
                                                {group.rows.map(({ rep, territory }) => (
                                                    <div key={`${rep.email}-${territory.key ?? territory.macroRegion}`} className="bg-white p-6 md:p-8 border border-industrial-200 rounded-2xl hover:border-accent-premium transition-colors group relative overflow-hidden shadow-sm hover:shadow-md">
